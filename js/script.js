...
// previeiosly selected topic (checking from locak storage)
const selectedTheme = localStorage.getItem('selected-theme')
const selectedIcon = localStorage.getItem('selected-icon')

// Weobtain the current theme that the interface has by validating the dark theme class
const getCurrentTheme = () => document.body.classList.contains(darkTheme)?'dark':'light'
const getCurrentIcon = () => document.body.classList.contains(iconTheme)?'uil-moon':'uil-sun'

// We need to validate if the user has previeiosly choosen a topic
if(selectedTheme){
...
new Typewriter('#typewriter', {
  strings: ['Darren Hung', 'Android-developer'],
  autoStart: true,
  loop: true,
  cursor: "|"
});
